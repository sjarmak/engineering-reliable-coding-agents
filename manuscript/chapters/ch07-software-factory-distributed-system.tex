...
\tightlist
\item
  Select one recent work item that produced or attempted an external effect.
\item
...
\item
  Name the authoritative owner of each fact and the boundary at which each consequential mutation is accepted.
\item
  Determine whether a stale attempt can still mutate the ledger, branch, artifact pointer, or external system.
\item
  Determine what the factory records when the external effect may have committed but acknowledgement is absent.
\item
  Verify that the accepted artifact and its verification record refer to the same immutable version.
\item
  Inject either a late completion or a lost acknowledgement, using the fault harness from Chapter 10, and retain the resulting event record.
\item
