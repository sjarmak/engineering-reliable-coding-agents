...
\label{ch10-replayable-traces-fault-injection-recovery}
\begin{quote}
\textbf{Evidence profile.} 1 strong \(\cdot\) 8 directional \(\cdot\) 1 direct preprint \(\cdot\) 0 corroborating evidence items across 2 developed practices (ERCA-097, ERCA-127).

\textbf{Chapter claim.} Recovery is a measured property.
\end{quote}
...
\hypertarget{what-a-transcript-cannot-answer}{%
...
\hypertarget{replay-from-a-changed-step}{%
...
\hypertarget{a-shared-vocabulary-and-its-limits}{%
...
\hypertarget{recovery-is-a-measured-property}{%
...
\hypertarget{specify-the-claim-before-the-kill}{%
...
\hypertarget{strike-the-interval-the-design-claims-to-protect}{%
...
\begin{figure}[htbp]
\centering
\includegraphics{figures/ch10-kill-points.pdf}
...
\hypertarget{distributed-ambiguity-faults}{%
\section{Distributed ambiguity faults}\label{distributed-ambiguity-faults}}
...
\caption{Distributed ambiguity faults, the invariant each tests, and the observable behavior an acceptable runtime must exhibit.}\tabularnewline
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
Delayed response delivered after the retry has started & I5 & The late response and the retry resolve to one logical effect; the effect ledger shows one committed \texttt{effect\_id} or an explicit \texttt{unknown\_external\_state}, never two commits. \\
Response lost after external commit & I5 & The runtime treats the call as uncertain, reconciles against external state before re-sending, and records the reconciliation outcome; no blind re-send. \\
Worker loses its lease but continues running & I2 & The protected mutation boundary rejects the superseded worker's writes by \texttt{ownership\_epoch}; the rejection appears in the trace as a fencing event, not a silent success. \\
Old worker's completion arrives after the new worker has succeeded & I3 & The stale completion is rejected by generation and attempt identity; logical state reflects the new attempt only, and the rejection is recorded. \\
Control plane restarts while workers continue & I6 & The restarted controller reconciles against durable worker and repository state before issuing new authority; no work item is double-assigned or orphaned by controller amnesia. \\
Control plane observes stale artifact state & I6 & Decisions taken on the stale view are detected at the next protected boundary and trigger reconciliation; the differential oracle shows no divergent committed transition. \\
Repository state changes between validation and publication & I6 & Publication fails closed: the runtime detects that the validated version is no longer the head, refuses to publish against changed state, and routes to re-validation or reconciliation. \\
Verifier times out, flakes, or contradicts itself on rerun & I8 & The runtime classifies the outcome as verifier failure, not software failure; no defect record is created from an infra timeout, and contradictory verdicts trigger rerun or escalation rather than acceptance of either verdict. \\
External service reachable from the worker but not the controller & I6 & The two components' contradictory observations are reconciled by declared authority; the controller does not mark the effect failed while the worker's committed effect record stands. \\
Schema or workflow version changes during recovery & I10 & Recovery either completes under an explicit compatibility path or fails closed with the version mismatch recorded; it never resumes by misreading old state under the new schema. \\
History compaction boundary crossed before the failure & I10 & Recovery from the compacted summary preserves the same invariant checks as recovery from full history; effects committed before compaction are still recognized and not repeated. \\
Recovery queue competes with new work & I10 & Recovery work and new admissions are both observable in queue metrics; recovery completes without starving admitted work indefinitely and without itself being starved invisibly. \\
Synchronized retry wave after a dependency recovers & I5, I10 & Retries are jittered or admission-limited; the dependency's recovery does not trigger a duplicate-effect burst, and each retried effect still deduplicates by its idempotency key. \\
\end{longtable}
...
\hypertarget{keep-the-result-attached-to-its-envelope}{%
...
\hypertarget{a-protocol-for-trusting-recovery}{%
...
The repository artifact \href{https://github.com/sjarmak/engineering-reliable-coding-agents/blob/main/protocols/recovery-fault-injection.md}{\texttt{protocols/recovery-fault-injection.md}} supplies the bounded run sequence, pass condition, and output manifest.
...
\textbf{Portable claim.} Recovery is a measured property.
