...
import { compileArxivArchive, sourceDateEpoch } from "./arxiv-compile.mjs";
...
test("the exact arXiv ZIP reproduces the recorded build under the pinned environment", async () => {
  ...
  assert.equal(actual.pdf.pages, manifest.manuscript.pages);
  ...
  assert.equal(actual.source_date_epoch, sourceDateEpoch(manifest.freeze_date));
  for (const field of [
    "archive_sha256",
    "pdf_sha256",
    "container_image",
