...
This human-readable edition presents all 206 reliability records in chapter context. For navigation, filtering, and graph exploration, use the [website companion](https://sjarmak.ai/books/engineering-reliable-coding-agents/companion).

This catalog contains 206 reliability records: 193 gated practices, including 56 developed in depth, plus 13 research leads. Of the 150 records not developed in the book, 137 are gated practices and 13 are leads preserved for investigation rather than recommendation. Grouping them by mechanism lets a reader scan a chapter’s neighborhood without erasing that distinction.
...
The 150 records not developed by any chapter appear as full compact entries. Every record has an action title and a stable identifier that can be cited, linked, or tracked across revisions. A compact entry then uses two or three paragraphs to describe the action, its mechanism, the evidence behind it, and where its use stops being justified. The entry closes on a boundary such as cost, dependency, transfer risk, failure condition, or limited support. Group labels and visibly different entry lengths distinguish pointers from compact entries at a glance.
...
Forty-two of the 150 compact entries carry a thin-support marker. Twenty-nine are gated practices with limited support. The other 13 are research leads outside the practice gate. None of the 42 should be read as a general recommendation; each is a prompt for a bounded investigation.
