...
The author did not commission external graders for v1. No reader responses, inter-rater statistics, or independent-calibration claim are part of this edition. [`status.json`](status.json) records that boundary in machine-readable form. The packet remains available so independent readers can apply the same instrument without treating the author's labels as ground truth.
...
For a future edition that includes commissioned grading, keep the generated report at this exact `calibration-report.json` path. A release declaring completed external grading verifies that the report covers 20 practices and 43 evidence items, identifies two or three unique readers, reports each required reader pair exactly once, and preserves the interpretation and disagreement fields. Do not create a calibration report for v1: its release state is disclosed nonperformance.
...
The absence of external grading is a disclosed limitation of v1, not a pending task or a claim that author-only challenge passes are equivalent to independent review.
